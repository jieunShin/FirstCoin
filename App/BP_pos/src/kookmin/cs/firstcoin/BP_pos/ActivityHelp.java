/**
 * @brief 도움말 액티비티
 * @details 서버로부터 도움말 관련 content를 받아와서 익스펜더블 리스트에 내용을 띄워줍니다.
 */
...
public class ActivityHelp extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		nDB = new DataHelp();// 서버가정
		// notification = server.getNotifications();//서버에서 모든 공지 제목들을 합한
		// String문자열 하나를 받음
		// StringTokenizer st = new StringTokenizer(notification,",");//토큰
		...
		listMain.setOnGroupClickListener(new OnGroupClickListener() {
			...
			public boolean onGroupClick(ExpandableListView parent, View v, int groupPosition, long id) {
				...
				// arrayContent.add(server.getNotifContent(groupPosition));//서버에서
				// groupPosition번째 공시내용물 받아와서 arrayContent에 추가
				httpclient = new DefaultHttpClient();
				StrictMode.setThreadPolicy(new StrictMode.ThreadPolicy.Builder().permitNetwork().build());
				httppost = new HttpPost("http://203.246.112.131/pos_help_content.php");

				try {
					StrictMode.setThreadPolicy(new StrictMode.ThreadPolicy.Builder().permitNetwork().build());
					String url = new String("http://203.246.112.131/pos_help_content.php?" + "title="
					...
					// ////////////////////////////////////////////db에서 받는 코드
					// 필요*********
					// arrayContent.add(response);//서버에서 groupPosition번째 내용물
					// 받아와서 arrayContent에 추가
					// / 데이터를 받아옴
					// 받아온 데이터 형태 : 메뉴이름,가격\n
				} catch (Exception e) {
