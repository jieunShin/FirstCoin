            ...
            google.setOnLoadCallback(drawChart);
              ...
              function drawChart() {
                var data = google.visualization.arrayToDataTable([
                  ...
                  <%
					...
					try {
						...
						//String sql2 = "select date_format(date, '%Y-%m') as month, sum(total) as total from statistic where merchant_id=? group by date_format(date, '%Y-%m') order by date asc limit 0,31 ";
						String sql2 = "select date_format(date, '%Y-%m') as month, sum(total) as total, sum(total_btc) as total_btc from statistic where merchant_id = ? group by date_format(date, '%Y-%m') order by date desc limit 0,31";
						...
						while(rs.next()) {
							...
							System.out.println(rs.getString("month") + " " + rs.getString("total"));
						
							if(rs.isLast()) {
								
					%>
								['<%= rs.getString("month") %>', <%= rs.getString("total") %>]
					<%
							}
							else {
					%>
                ...
                var options = {
                  ...
                  series:{
                	  0: {lineWidth:2, pointSize:8, color:'#008800', pointShape:'diamond'}
                  }
                };
